    ...
    <footer className="relative z-10 bg-[#030008]/20 backdrop-blur-xl border-t border-white/5 py-24 mt-20">
      <div className="container mx-auto px-6 md:px-12 flex flex-col md:flex-row justify-between items-start gap-16">
        ...
        <div className="flex flex-col sm:flex-row gap-16 md:w-2/3 md:justify-end">
          <div>
            <h4 className="text-xs font-semibold tracking-[0.2em] uppercase mb-8 text-[var(--foreground)]">{t("mindCreations")}</h4>
          ...
          <div>
            <h4 className="text-xs font-semibold tracking-[0.2em] uppercase mb-8 text-[var(--foreground)]">{t("communicate")}</h4>
            <ul className="space-y-6">
              <li>
      ...
      <div className="container mx-auto px-6 md:px-12 mt-20 pt-10 border-t border-black/5 flex flex-col md:flex-row items-center justify-between gap-6">
        <p className="text-xs tracking-wider text-gray-400 uppercase">© {new Date().getFullYear()} <Elev8LogoText textClassName="uppercase" iconClassName="w-[0.55em] h-[0.85em] mx-[0.1em] opacity-80" /> INCORPORATION. {t("rights")}.</p>
        <div className="flex gap-8 text-xs tracking-wider text-gray-400 uppercase">
          <a href="#" className="hover:text-[var(--foreground)] transition-colors duration-300">{t("privacyPolicy")}</a>
          <a href="#" className="hover:text-[var(--foreground)] transition-colors duration-300">{t("termsOfService")}</a>
        </div>
      </div>
    </footer>
  );
